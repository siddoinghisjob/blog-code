...
function generateBlogComponent(htmlContent, metadata, blogId) {
  ...
  const escapedDescription = (metadata.excerpt || `${escapedTitle} - Blog post by ${escapedAuthor}`).replace(/[`$]/g, "\\$&");
  const cardImage = metadata.cardImage || null; // Get card image URL from metadata, default to null
...
export const head: DocumentHead = {
  ...
  meta: [
    ...
    { name: "twitter:card", content: ${cardImage ? '"summary_large_image"' : '"summary"'} },
    ...
    ${cardImage ? `{ name: "twitter:image", content: "${cardImage}" },` : ''}
...
function updateBlogJson(id, metadata) {
  ...
  const blogEntry = {
    ...
    tags: metadata.tags || [],
    excerpt: metadata.excerpt || "", // Keep excerpt here
